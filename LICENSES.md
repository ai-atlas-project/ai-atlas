# Licensing

AI Atlas public content is licensed under Creative Commons Attribution-ShareAlike 4.0 International unless otherwise stated.

## Summary

| Material | License |
|---|---|
| Taxonomy content, Markdown, JSON, Obsidian notes, Markmap exports, Mermaid exports, and public documentation | CC BY-SA 4.0 |
| AI Atlas name, logo, brand assets, official paid print/PDF designs | Not automatically licensed for commercial reuse unless explicitly stated |

## Content license

The open taxonomy and educational content are licensed under Creative Commons Attribution-ShareAlike 4.0 International.

See:

- [CONTENT_LICENSE](CONTENT_LICENSE)

Recommended attribution:

> AI Atlas by Alyaksandr Stzhalkouski, licensed under CC BY-SA 4.0.

## Brand and official commercial products

The open taxonomy may be used under the content license, but the AI Atlas name, logo, visual identity, and official paid print/PDF designs are not automatically licensed for commercial reuse unless explicitly stated.

This allows the project to remain open for community contribution while still preserving the possibility of official commercial products such as:

- print-ready posters,
- premium PDF editions,
- curated visual maps,
- official design packs.
